        ...
        let ctx = gsap.context(() => {
            ...
            ScrollTrigger.create({
                ...
                animation: gsap.timeline()
                    .to(videoRef.current, { scale: 1.03, ease: "none" }, 0)
                    .to([eyebrowRef.current, line1Ref.current, line2Ref.current, line3Ref.current, copyRef.current, ctaRef.current], {
                        y: -30,
                        opacity: 0,
                        stagger: 0.05,
                        ease: "none"
        ...
        >
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 flex flex-col justify-center h-full pt-16">
                <div className="w-full md:w-[42%] flex flex-col justify-center">
                    {/* Eyebrow */}
                    <div ref={eyebrowRef} className="text-xs md:text-sm font-medium tracking-widest text-zinc-800 mb-6 md:mb-8">
                        FUSION TECH
                    </div>

                    {/* Headline (Character-by-character GSAP animation) */}
                    <h1 className="font-instrument text-3xl md:text-4xl lg:text-5xl xl:text-[3.75rem] text-[#111111] leading-[0.9] tracking-[-0.04em] mb-8" style={{ perspective: "1000px" }}>
                        <span className="block overflow-hidden pb-2 font-medium"><div ref={line1Ref}>{splitText("INTELLIGENCE.")}</div></span>
                        <span className="block overflow-hidden pb-2 font-normal"><div ref={line2Ref}>{splitText("BUILT INTO")}</div></span>
                        <span className="block overflow-hidden pb-2 font-medium"><div ref={line3Ref}>{splitText("YOUR SPACE.")}</div></span>
                    </h1>

                    {/* Supporting Copy */}
                    <p ref={copyRef} className="text-zinc-600 text-sm md:text-base max-w-[380px] mb-10 leading-relaxed">
                        Lighting, climate, security and entertainment — quietly connected around the way you live.
                    </p>

                    {/* CTA */}
                    <a ref={ctaRef} href="#explore" className="inline-flex items-center text-xs md:text-sm font-medium text-zinc-900 group tracking-wide">
